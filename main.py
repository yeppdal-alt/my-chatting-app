...
#    - 말투별로 시스템 프롬프트 문구를 미리 준비해둔다.
#    - 어떤 말투를 고르더라도 "순수 한국어로만 답해"는 공통으로 지켜지게 한다.
# -----------------------------
TONE_OPTIONS = {
    "친절한 오빠": (
        "너는 다정하고 친절한 오빠처럼 편안한 말투로 설명해주는 데이터 분석 선생님이야. "
        "부드럽고 든든하게, 알기 쉽게 알려줘. 반드시 순수 한국어로만 답해."
    ),
    "애교많은 전문가 언니": (
        "너는 애교 많고 센스있는 전문가 언니처럼 말하는 데이터 분석 선생님이야. "
        "실력은 확실하지만 말투는 발랄하고 애교 섞인 표현을 자연스럽게 사용해. 반드시 순수 한국어로만 답해."
    ),
    "귀여운 친구": (
        "너는 귀엽고 발랄한 친구처럼 편하게 설명해주는 데이터 분석 선생님이야. "
        "친근한 반말과 재미있는 표현을 곁들여서 설명해. 반드시 순수 한국어로만 답해."
    ),
}

# -----------------------------
# 3-1) 사이드바에 '말투 고르기' 메뉴 넣기
#      - st.sidebar.radio는 라디오 버튼 형태의 선택지를 사이드바에 그려준다.
#      - key="tone_choice"로 지정해두면 선택값이 session_state["tone_choice"]에
#        자동으로 저장되고, 페이지가 다시 그려져도 선택이 유지된다.
#      - system 프롬프트는 매번 대화를 보낼 때 새로 만들기 때문에,
#        말투를 바꾸면 다음 답변부터 바로 새 말투가 적용된다.
# -----------------------------
st.sidebar.header("⚙️ 설정")
tone_choice = st.sidebar.radio("말투 고르기", list(TONE_OPTIONS.keys()), key="tone_choice")
SYSTEM_PROMPT = TONE_OPTIONS[tone_choice]
...
# -----------------------------
# 4-1) 사이드바에 '대화 지우기' 버튼 넣기
#      - 버튼을 누르면 세션에 쌓인 대화 기록을 완전히 비운다.
#      - st.rerun()으로 화면을 즉시 새로고침해서 지운 결과를 바로 보여준다.
# -----------------------------
